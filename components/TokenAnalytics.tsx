...
export const TokenAnalytics: React.FC<TokenAnalyticsProps> = ({ ca, initialMetrics }) => {
  ...
  const getHeldSince = (rank: number, seedStr: string): { date: string, isOldfag: boolean, daysHeld: number } => {
      let daysHeld = 0;
      if (rank <= 5) { daysHeld = 45; } else { daysHeld = (seedStr.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0) % 40) + 1; }
      const date = new Date(); 
      date.setDate(date.getDate() - daysHeld);
      const mm = String(date.getMonth() + 1).padStart(2, '0'); 
      const dd = String(date.getDate()).padStart(2, '0');
      return { date: `${mm}/${dd}`, isOldfag: daysHeld > 14, daysHeld };
  };
    ...
    try {
        ...
        if (accounts.length > 0) {
            const realHolders: Holder[] = accounts.map((acc: any, index: number) => {
                ...
                return { rank: index + 1, address: acc.address, amount: amount, percentage: parseFloat(pct.toFixed(2)), value: amount * liveMetrics.price, tag: tag, heldSince: date, isOldfag, daysHeld, hasSold, diamondHands };
            });
  ...
  const generateLiveHolders = () => {
    ...
    const holders: Holder[] = fallbackData.map((d, i) => {
        ...
        const diamondHands = !hasSold && daysHeld > 7;
        return { rank: i + 1, address: d.addr, amount: amount, percentage: d.pct, value: amount * liveMetrics.price, tag: d.tag, heldSince: i < 3 ? "Inception" : "Early Access", isOldfag: daysHeld > 14, daysHeld, hasSold, diamondHands };
    });
    return holders;
  };
  ...
  return (
    // High Visibility Update: Darker background, solid color, no blur filters that mud the text
    <div className="relative w-full max-w-[98%] 2xl:max-w-[1600px] mx-auto mt-8 mb-8 border-[2px] border-cyan-800 bg-[#02040a] font-tech text-white overflow-hidden shadow-2xl rounded-lg ring-1 ring-cyan-900/50">
      ...
      <div className="flex bg-[#121926] border-b border-cyan-800 p-1 gap-1 text-[10px] flex-wrap font-tech relative z-20">
         <button onClick={() => setActiveTab('holders')} className={`px-3 py-2 border ${activeTab === 'holders' ? 'bg-cyan-600 border-cyan-400 text-white font-bold shadow-md' : 'bg-slate-800 border-slate-700 text-slate-400 hover:text-white hover:bg-slate-700'} cursor-pointer uppercase tracking-wider transition-all`}>CREW MANIFEST</button>
